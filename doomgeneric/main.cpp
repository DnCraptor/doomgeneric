...
uint8_t __aligned(4) FRAME_BUF[DOOMGENERIC_RESX * DOOMGENERIC_RESY] = { 0 };
...
void __scratch_x("render") render_core() {
    ...
    graphics_init();
    graphics_set_buffer(FRAME_BUF, DOOMGENERIC_RESX, DOOMGENERIC_RESY);
...
extern "C" uint8_t* get_line_buffer(int line) {
    return FRAME_BUF + DOOMGENERIC_RESX * line;
}
...
extern "C" void DG_Init(void) {
    DG_ScreenBuffer = (pixel_t*)FRAME_BUF;
}
...
extern "C" void DG_GetMouseMove(int *x, int *y) {
#ifdef KBDUSB
    repeat_me_for_input();
#endif
    *x = cumulative_dx;
...
extern "C" void DG_SetPalette(unsigned char palette[768]) {
    uint8_t *p = palette;
    for (int i = 0; i < 256; i++) {
...
static void create_argv() {
    FIL* f = new FIL();
    if (f_open(f, "/doom/argv.conf", FA_READ) == FR_OK) {
        UINT br = 0;
...
*/
	...
	//Sys_Printf ("doomgeneric_Create done\n");
...
static char* open_config(UINT* pbr) {
    ...
    if (f_read(&f, buff, file_size, pbr) != FR_OK) {
        ///printf("Failed to read /doom/doom.conf\n");
        free(buff);
        buff = 0;
    }
    f_close(&f);
